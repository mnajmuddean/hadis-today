  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-r from-[#C9DABF] to-[#9CA986] overflow-hidden">
      <header className="w-full bg-[#808D7C] text-white py-4 text-center">
        ...
        <p className="text-xl font-sans mt-2">Explore the Wisdom of Hadith</p>
      </header>
            ...
            <CarouselContent>
              {hadiths.map((hadith, index) => (
                <CarouselItem key={index} className="flex items-center justify-center">
                  <Card className="w-full max-h-[80vh] overflow-auto bg-[#C9DABF]/90 shadow-xl hover:shadow-2xl transition-shadow duration-300 border-2 border-[#808D7C]">
                    <CardHeader className="bg-[#808D7C] text-white sticky top-0 z-10">
                      <CardTitle className="text-xl font-arabic font-mono">{hadith.hadithTitle || 'Title Not Yet Available'}</CardTitle>
                      <CardDescription className="text-white/80 font-mono">{hadith.hadithNarrator || 'Narrator Not Yet Available'}</CardDescription>
                    </CardHeader>
                    <CardContent className="pt-6">
                      <p className="text-right font-arabic text-lg leading-loose text-[#5F6F65]">{hadith.hadithArabic || 'Hadith Arabic Not Yet Available'}</p>
                      <p className="mt-4 text-sm italic text-[#5F6F65] font-mono">{hadith.hadithEnglish || 'Hadith English Not Yet Available'}</p>
                    </CardContent>
                    <CardFooter className="bg-[#9CA986] text-[#5F6F65] font-semibold sticky bottom-0 flex items-center justify-between">
                      <p className='mt-5 font-mono'>{hadith.hadithBookName || 'Hadith Book Name Not Yet Available'}</p>
                      <span className={`px-2 py-1 rounded mt-4 ${getStatusColor(hadith.hadithStatus)}`}>
      ...
      <footer className="w-full bg-[#808D7C] text-white py-4 text-center">
        <p className="text-sm font-mono">Created by mnajmuddean</p>
      </footer>
    </div>
  );
}
